...
struct Emitter: ff_node_t<float> {
	Emitter( ff_loadbalancer *const lb,  int max_iter, float tollerance,
		std::vector<float>& x, std::vector<float>& x_new, int pd):
		lb(lb),max_iter(max_iter), tollerance(tollerance), pd(pd) {}
	ff_loadbalancer *const lb;
	int max_iter, pd;
	int k = 0, received = 0;
	float tollerance;  
	std::vector<float>& x, x_new;
...
int main(int argc, char *argv[]){
	if (argc < 2){
		std::cout << "Usage: " << argv[0] << " <matrix_dimension> <PAR_DEGREE> <diag_dominant> <iterations> <tollrance> \n" << std::endl;
		return -1;
	}
	int n = atoi(argv[1]);
	int pd = atoi(argv[2]);
	...
	std::cout << "\nPrinting B: "  << std::endl;
	print_vec(b,n);
	bool sequential = false;  //TODO: take from command line
	if (sequential)
		...
		*/
		...
		Emitter E(farm.getlb(), iter, tollerance, x, x_new, pd);
		farm.add_emitter(E);
